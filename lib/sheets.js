...
// 5S: today value = average Final Score within the selected range (default: latest date overall)
...
// Kaizen: count of rows with Created Date (col K) in the selected month's YYYY-MM (default: current month)
function computeKaizen(rows, monthKey){
  const header = rows[0]||[];
  const cIdx = colIdx(header, 'Created Date', 10);
  const now = new Date();
  const curY = now.getFullYear(), curM = String(now.getMonth()+1).padStart(2,'0');
  const month = monthKey || (curY+'-'+curM);
  let mtd = 0, total = 0;
  for(let i=1;i<rows.length;i++){
    const row = rows[i];
    ...
    if(d && d.slice(0,7)===month) mtd++;
  }
  return { mtdCount:mtd, total, month, target:KAIZEN_TARGET };
}
...
async function fetchFiveSKaizen(key, from, to){
  ...
  try{ const rows = await fetchCsv(cfg.kaizen.id, cfg.kaizen.gid); out.kaizen = { ...computeKaizen(rows, (to||'').slice(0,7) || undefined), target:KAIZEN_TARGET }; }
